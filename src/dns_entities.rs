...
impl DnsAnswer {
    fn new(name: String, atype: u16, aclass: u16, ttl: u32, rdlength: u16, rdata: Vec<u8>) -> DnsAnswer {
        return DnsAnswer {
            name: name,
            atype: atype,
            aclass: aclass,
...
impl DnsName {
    ...
    fn parse_label(packet: &mut DnsPacket, len: usize) -> Result<String, FromUtf8Error> {
        ...
        for i in 0..len {
            match packet.next_u8() {
                Some(0) | None => {
                    warn!("Found terminating byte or end of buffer before len ({}) bytes read", len);
                    break;
                },
                Some(byte) => label.insert(i, byte)
            }
        }
        debug!("label bytes {:?}", label);
        let label_str = String::from_utf8(label);
        debug!("label: {:?}", label_str);
        return label_str;
    }
