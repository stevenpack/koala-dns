        ...
        match self.state {
            ...
            State::ForwardRequest => {
                ...
                match self.upstream_socket
                          .send_to(&mut bytes::SliceBuf::wrap(self.query_buf.as_slice()),
                                   &upstream_addr) {
                    Ok(Some(_)) => {
                        // todo log bytes
            ...
            State::AnswerClient => {
                assert!(events.is_writable());
                match udp_server.send_to(&mut SliceBuf::wrap(&self.response_buf),
                                         &self.client_addr) {
            ...
            State::Close => {                
                error!("Should not be firing socket events when closed. {:?}", token);
            }
        }
    }
...
impl MioServer {
    ...
    fn receive(&self, socket: &UdpSocket) -> Option<(SocketAddr, Vec<u8>)> {
        ...
        match socket.recv_from(&mut buf) {
            Ok(Some(addr)) => return Some((addr, buf)),                            
